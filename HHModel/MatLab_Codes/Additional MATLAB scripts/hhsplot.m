...
figure(2);
set(2,'Position',[200 150 620 600],'Color','w');

subplot(2,2,1),plot(ics(:,1),ics(:,2));
% set(gca,'Color','k','XColor','k','YColor','k');
xlabel('time (ms)','Color','k'),ylabel('Ic (microamp/cm^2)','Color','k'),axis([to tf 1.1*min([0 amp1 amp2]) 1.1*max([amp1 amp2])]);
title('Stimulus current','Color','k');
 subplot(2,2,2),plot(t,y(:,1));
% set(gca,'Color','k','XColor','k','YColor','k');
xlabel('time (ms)','Color','k'),ylabel('V_m (mV)','Color','k'),axis([to tf -100 50]);
title('Membrane potential','Color','k');
subplot(2,2,3),plot(t,gi(:,1),'r-',t,gi(:,2),'b-');
% set(gca,'Color','k','XColor','k','YColor','k');
xlabel('time (ms)','Color','k'),ylabel('G (mS/cm^2)','Color','k'),axis([to tf -5 40]);
title('Ionic conductances','Color','k'),legend('G_{Na}','G_{K}');
subplot(2,2,4),plot(t,-0.001*jm(:,1),'r-',t,-0.001*jm(:,2),'b-',t,-0.001*jm(:,3),'g-');
% set(gca,'Color','k','XColor','k','YColor','k');
xlabel('time (ms)','Color','k'),ylabel('J (mA/cm^2)','Color','k'),axis([to tf -1 1]);
title('Ionic currents','Color','k'),legend('J_{Na}','J_{K}','J_{Cl}');
